...
export async function createProduct({ category_id, title, description, price, location,
                                      model, ownership, year, kilometers, expected_price, images = [] }) {
  const [result] = await pool.query(
    `INSERT INTO products 
     (category_id, title, description, price, location, model, ownership, year, kilometers, expected_price) 
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [category_id || null, title, description, price, location,
     model || null, ownership || null, year || null, kilometers || null, expected_price || null])
...
export async function createSubmission(data) {
  const { seller_name, seller_phone, seller_whatsapp, seller_email,
          ...
          model, ownership, year, kilometers, expected_price } = data
  const [result] = await pool.query(`
