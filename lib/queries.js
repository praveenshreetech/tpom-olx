...
export async function getAllProducts({ category, search, limit = 20, offset = 0 } = {}) {
  let sql = `
...
export async function createProduct({ category_id, title, description, price, location, images = [] }) {
  const [result] = await pool.query(
    'INSERT INTO products (category_id, title, description, price, location) VALUES (?, ?, ?, ?, ?)',
...
export async function createSubmission(data) {
  const { seller_name, seller_phone, seller_whatsapp, seller_email,
          product_title, description, price, location, image_urls, contact_method } = data
  const [result] = await pool.query(`
    ...
    [seller_name, seller_phone, seller_whatsapp || null, seller_email || null,
     ...
     image_urls || null, contact_method])
  return result.insertId
}

export async function getAllSubmissions() {
  const [rows] = await pool.query(
    'SELECT * FROM seller_submissions ORDER BY created_at DESC')
  return rows
}
