...
export async function getAllProducts(data = {}) {
  ...
  let sql = `
    SELECT p.id, p.title, p.price, p.location, p.views_count, p.created_at, p.status,
           c.name AS category,
...
export async function getProductById(id) {
  const [[product]] = await pool.query(`
    ...
    FROM products p
    LEFT JOIN categories c ON c.id = p.category_id
    WHERE p.id = ? AND p.status IN ('active', 'sold')`, [id])
...
export async function deleteSubmission(id) {
  await pool.query('DELETE FROM seller_submissions WHERE id = ?', [id])
}
