...
export async function POST(request) {
  try {
    ...
    console.log("Incoming Inquiry Data:", body)
    const { product_id, product_name, product_price, buyer_name, buyer_phone, buyer_email, message, contact_method } = body

    if (!product_id || !product_name || !product_price || !buyer_name || !message) {
      return NextResponse.json({ error: 'product_id, product_name, product_price, buyer_name and message are required' }, { status: 400 })
    }

    // Save to database
    const id = await createInquiry({
      product_id, product_name, product_price, buyer_name, buyer_phone, buyer_email, message,
